...
void SendMsg(Net id,string mg149) {
    printf("%s",mg149.c_str());
    NetSend(id,("$:o"+mg149+"$:e").c_str());
    }
void SendMsg(vector<Net> ids,string mg149) {
    printf("%s",mg149.c_str());
    for(Net nt:ids)
        NetSend(nt,("$:O"+mg149+"$:e").c_str());
    }
/*
void SendMsg2(Net id,string mg149) {
    printf("%s",mg149.c_str());
    NetSend(id,("$:O"+mg149+"$:e").c_str());
    }
void SendMsg2(vector<Net> ids,string mg149) {
    printf("%s",mg149.c_str());
    for(Net nt:ids)
        NetSend(nt,("$:O"+mg149+"$:e").c_str());
    }*/
void SendCmd(Net id,string mg149) {
    printf("%s",mg149.c_str());
    NetSend(id,("$:r"+mg149+"$:e").c_str());
    }
void SendCmd(vector<Net> ids,string mg149) {
    printf("%s",mg149.c_str());
    for(Net nt:ids)
        NetSend(nt,("$:r"+mg149+"$:e").c_str());
    }
int GetNum(Net id) {
    NetSend(id,"$:P");
    Time tim=GetTime();
