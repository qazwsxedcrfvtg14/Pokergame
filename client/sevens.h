...
class sevens_element:public poker_game {
    ...
    public:
        ...
        void start_a_game(vector<Net>pl_lis);
        void clear_score();
    };
...
void sevens_element::start_a_game(vector<Net>pl_lis_fk) {
    pl_lis=pl_lis_fk;
    bool ai[4]= {0,0,0,0};
    initial();
    while(player_card_index[nowplay]) {
        if(ai[nowplay]) {
            if(can_play()) {
                ...
                card_picture(player[nowplay][ti]).printout(pl_lis);
                }
            else {
                int tmax = -2147483647;
                int ti = 0;
    ...
    for(int i = 0; i < 4; i++) {
        ...
        SendMsg(pl_lis,player_name[i]+"蓋\的牌\n");
        res.printout(pl_lis);
        }
    for(int i = 0; i < 4; i++) {
        sum += thisscore[i];
    ...
    score[tempi] -= sum;
    for(int i = 0; i < 4; i++) {
        SendMsg(pl_lis,player_name[i]+"的分數為"+IntToStr(score[i]));
        }
    }
